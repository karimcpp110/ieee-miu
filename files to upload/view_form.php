...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (in_array($field['type'], ['mcq', 'true_false', 'short_answer', 'matching', 'ordering'])) {
            ...
            if ($field['type'] === 'mcq') {
                foreach ($field['options'] as $opt) {
                    if (!empty($opt['is_correct'])) $correctValue = $opt['text'];
                    if (!empty($opt['is_correct']) && $userAnswer == $opt['text']) {
                        $isCorrect = true;
                    }
                }
            } elseif ($field['type'] === 'true_false') {
                $correctValue = $field['correct_answer'] ?? '';
                if ($userAnswer == $correctValue) $isCorrect = true;
            } elseif ($field['type'] === 'short_answer') {
                $correctValue = $field['correct_answer'] ?? '';
                if (strcasecmp(trim($userAnswer), trim($correctValue)) === 0) $isCorrect = true;
            } elseif ($field['type'] === 'matching') {
                ...
                $correctValue = implode(' | ', $expectedMatches);
                if ($matchCount == count($field['pairs'])) $isCorrect = true;
            } elseif ($field['type'] === 'ordering') {
                $correctValue = implode(' | ', $field['items']);
                if ($userAnswer == $correctValue) $isCorrect = true;
            }
            ...
            if ($isCorrect) {
                ...
                $incorrectInsights[] = [
                    ...
                    'student_ans' => $userAnswer,
                    'correct_ans' => $correctValue
                ];
            }
        }
    }
    ...
    if ($studentId && $data['Passing_Status'] === 'Passed') {
        ...
        // 🔔 Fire an in-app notification for passing
        require_once 'Notification.php';
        $notiSystem = new Notification();
        $notiSystem->createForStudent(
            $studentId,
            '🏆 Exam Passed!',
            "You scored {$scorePercentage}% on \"{$form['title']}\"." . ($linkedCourse ? " Certificate for \"{$linkedCourse['title']}\" is now available!" : ''),
            'success'
        );
    }
}
...
<body>
        ...
        <div class="glass-panel form-viewer-card">
            ...
            <?php if ($msg): ?>
                <div class="success-state">
                    ...
                    <!-- AI Assistant Hook -->
                    <div class="ai-tutor-panel" id="aiTutorPanel" style="display: none; margin: 2rem 0; padding: 1.5rem; background: rgba(0, 243, 255, 0.05); border: 1px solid var(--primary-neon); border-radius: 12px; text-align: left;">
                        <h3 style="color: var(--primary-neon); margin-bottom: 1rem; display: flex; align-items: center; gap: 10px;">
                            <i class="fas fa-robot"></i> AI Tutor Feedback
                        </h3>
                        <div id="aiThinking" style="color: var(--text-muted); font-size: 0.9rem;">
                            <i class="fas fa-circle-notch fa-spin"></i> Analyzing your responses...
                        </div>
                        <div id="aiSummary" style="display: none; color: var(--text-main); line-height: 1.6; font-size: 1.05rem;">
                            <!-- Typewriter text goes here -->
                        </div>
                        <div id="aiEngineLabel" style="margin-top: 1rem; font-size: 0.75rem; color: var(--text-muted); text-align: right; display:none;">
                            Powered by <span id="engineIdName">Heuristic AI</span> ⚡
                        </div>
                    </div>
                    
                    <a href="index.php" class="btn btn-primary mt-1">Return Home</a>

                    <script>
                        // Execute AI Hook if data exists
                        document.addEventListener("DOMContentLoaded", () => {
                            const insights = <?= json_encode($incorrectInsights ?? []) ?>;
                            const score = <?= json_encode($scorePercentage ?? 100) ?>;
                            
                            const aiPanel = document.getElementById('aiTutorPanel');
                            const aiThinking = document.getElementById('aiThinking');
                            const aiSummary = document.getElementById('aiSummary');
                            const aiLabel = document.getElementById('aiEngineLabel');
                            const engineIdName = document.getElementById('engineIdName');

                            if (aiPanel) {
                                aiPanel.style.display = 'block';
                                
                                fetch('api/v1/ai_assistant.php', {
                                    method: 'POST',
                                    headers: {
                                        ...
                                        const typeWriter = setInterval(() => {
                                            if (i < text.length) {
                                                ...
                                                if(data.engine) {
                                                    ...
                                                    engineIdName.textContent = data.engine === 'openai' ? 'OpenAI GPT' : 'HyperDrive AI (Local)';
                                                }
                                            }
                                        }, 20); // typing speed
                                    } else {
                                        aiSummary.innerHTML = "<em>AI Tutor is currently resting. Great job finishing the exam!</em>";
                                    }
                                })
                                .catch(err => {
                                    aiThinking.style.display = 'none';
                                    aiSummary.style.display = 'block';
                                    aiSummary.innerHTML = "<em>AI Tutor connection failed. Keep learning!</em>";
                                });
                            }
                        });
                    </script>
            ...
            <?php else: ?>
                ...
                <form method="POST" class="styled-form dynamic-form" id="examForm">
                    <?php 
                    // Shuffle logic for Exams
                    $shuffledIndexes = array_keys($fields);
                    if ($form['type'] === 'exam') {
    ...
    document.addEventListener("visibilitychange", function() {
        if (document.hidden) {
            tabSwitchWarnings++;
            if (tabSwitchWarnings >= maxWarnings) {
                alert("🚨 EXAM TERMINATED: Maximum cheating warnings (" + maxWarnings + ") reached. Your exam is auto-submitting.");
                document.getElementById('examForm').submit();
            } else {
                alert("⚠️ WARNING " + tabSwitchWarnings + "/" + maxWarnings + ": \nYou have navigated away from the exam window. Continued tab-switching is prohibited and will result in auto-failure.");
            }
        }
    });

    // Prevent right click and copy for exam integrity
    document.addEventListener('contextmenu', event => event.preventDefault());
    document.addEventListener('copy', event => {
        event.preventDefault();
        alert("⚠️ Copying text is disabled during this exam.");
    });
    <?php endif; ?>
    </script>
