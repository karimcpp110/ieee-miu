...
try {
    ...
    $studCols = $res->fetchAll(PDO::FETCH_COLUMN);
    if (!in_array('api_key', $studCols)) {
        echo "<p>Column 'api_key' is MISSING in 'students'. Adding it now...</p>";
        $db->query("ALTER TABLE students ADD COLUMN api_key VARCHAR(100) UNIQUE DEFAULT NULL AFTER password");
        echo "<div class='status success'>SUCCESS: 'api_key' column added to students.</div>";
    }
    ...
    // 6. Check users table for RBAC and Security
    ...
    if (!in_array('api_key', $userCols)) {
        echo "<p>Column 'api_key' is MISSING. Adding it now...</p>";
        $db->query("ALTER TABLE users ADD COLUMN api_key VARCHAR(100) UNIQUE DEFAULT NULL AFTER password");
        echo "<div class='status success'>SUCCESS: API Key column added.</div>";
    }
    ...
    echo "<h3>9. Ensuring Logs, Analytics & Settings Tables...</h3>";
    ...
    echo "<h3>10. Executing Analytics Checks...</h3>";
    echo "<div class='status info'>Analytics and Queue Tables ensured.</div>";
    ...
    echo "<h3>Check Complete with Error!</h3>";
