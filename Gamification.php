...
class Gamification
{
    ...
    public function completeCourse($userId, $courseId)
    {
        ...
        // Update user stats (Assuming XP is calculated as 150 per course)
        // Since we may not have an XP column yet, we simulate it via courses_completed.
    ...
    public function getUserLevel($coursesCompleted) {
        if ($coursesCompleted >= 10) return 'Master';
        if ($coursesCompleted >= 7) return 'Expert';
        if ($coursesCompleted >= 4) return 'Adept';
        if ($coursesCompleted >= 1) return 'Apprentice';
        return 'Novice';
    }

    public function getUserGamificationProfile($userId) {
        $stats = $this->db->query("SELECT courses_completed, last_login FROM user_stats WHERE user_id = ?", [$userId])->fetch();
        $courses = $stats ? (int)$stats['courses_completed'] : 0;
        
        return [
            'courses_completed' => $courses,
            'xp' => $courses * 150, // 150 XP per course
            'level' => $this->getUserLevel($courses),
    ...
    public function getLeaderboard($limit = 10)
    {
        $limit = (int) $limit;
        try {
            $board = $this->db->query("SELECT s_acc.full_name as username, stat.courses_completed, stat.events_attended 
            ...
            // Map XP and Level dynamically
            foreach($board as &$user) {
                $user['xp'] = $user['courses_completed'] * 150;
                $user['level'] = $this->getUserLevel($user['courses_completed']);
            }
            return $board;
        } catch (Exception $e) {
            return [];
